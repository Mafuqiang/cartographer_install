...
/*
 * added by mafuqiang: reset the class Mapbuilder;
 */
void MapBuilderBridge::ResetMapbuilder(std::unique_ptr<cartographer::mapping::MapBuilderInterface> new_map_builder){
    map_builder_.reset();
    map_builder_ = std::move(new_map_builder);
...
void MapBuilderBridge::HandleSubmapQuery(
  ...
  if (!error.empty()) {
    LOG(ERROR) << error;
    response.status.code = cartographer_ros_msgs::StatusCode::NOT_FOUND;    // 状态码？未发现；
...
std::set<int> MapBuilderBridge::GetFrozenTrajectoryIds() {
  ...
  const auto node_poses = map_builder_->pose_graph()->GetTrajectoryNodePoses(); // 位姿图里面的节点姿态应该与地图重载相关；
...
cartographer_ros_msgs::SubmapList MapBuilderBridge::GetSubmapList() {
  ...
  submap_list.header.frame_id = node_options_.map_frame;    // map的frame id？？
  for (const auto& submap_id_pose :
       map_builder_->pose_graph()->GetAllSubmapPoses()) {   // 从位姿图中获取所有的子图位姿；
...
// 获取轨迹状态，轨迹状态包含了轨迹参数、局部slam数据、局部SLAM到全局的变换、发布的frame、时间戳等；
...
visualization_msgs::MarkerArray MapBuilderBridge::GetTrajectoryNodeList() {
  ...
  for (const auto& constraint : constraints) {
    if (constraint.tag == cartographer::mapping::PoseGraph::Constraint::INTER_SUBMAP) {
      if (constraint.node_id.trajectory_id == constraint.submap_id.trajectory_id) { //？？？   //和地图重载或多机协同相关？
        trajectory_to_last_inter_submap_constrained_node[constraint.node_id.trajectory_id] =
            std::max(trajectory_to_last_inter_submap_constrained_node.at(
                         constraint.node_id.trajectory_id), constraint.node_id.node_index); // 和原来的对比，如果大于原来的就代替原来的，
                                                                                            // 索引号越大说明越新；
      } else {
        trajectory_to_last_inter_trajectory_constrained_node[constraint.node_id.trajectory_id] =
                std::max(trajectory_to_last_inter_submap_constrained_node.at(
