...
using carto::transform::Rigid3d;
    ...
    Node::Node(
        ...
        //added by mafuqiang: startup the services of start_create_map, finish_create_map, start_localization;
        service_servers_.push_back(node_handle_.advertiseService(
                kStartCreateMapServiceName, &Node::HandleStartCreateMap, this));

        service_servers_.push_back(node_handle_.advertiseService(
                kFinishCreateMapServiceName, &Node::HandleFinishCreateMap, this));

        service_servers_.push_back(node_handle_.advertiseService(
                kStartLocalizationServiceName, &Node::HandleStartLocalization, this));

        service_servers_.push_back(node_handle_.advertiseService(
                kStartLocalizationFromPbstreamServiceName, &Node::HandleStartLocalizationFromPbstream, this));

        service_servers_.push_back(node_handle_.advertiseService(
                kFinishLocalizationServiceName, &Node::HandleFinishLocalization, this));

        create_map_trajectory_id = 0;
        localization_trajectory_id = 1;

        start_create_map_controller = true;
        //added end;
...
/*
 * added by mafuqiang: reset the node when restart the service of "start_create_map",
 * it is the key to reset the 'map_builder_', and clear the 'extrapolators_' and 'sensor_samplers'.
 */
void Node::ResetNode(){
    ...
    carto::common::MutexLocker lock(&mutex_);
    map_builder_bridge_.ResetMapbuilder(std::move(new_map_builder));
...
void Node::PublishTrajectoryStates(const ::ros::WallTimerEvent& timer_event) {
  carto::common::MutexLocker lock(&mutex_);
  // 大部分东西是从map_builder_bridge这里获取的，所以这应该是ros和lib之间的接口；
...
//added by mafuqiang;
void Node::SetTrajectoryOptions(TrajectoryOptions &trajectory_options)
{
    trajectory_options_mafuqiang = trajectory_options;
}

void Node::SetDirectoryAndBasename(std::string &directory,
        std::string &basename, std::string &basename_localization){
    configuration_directory = directory;
    configuration_basename  = basename;
    configuration_basename_localization = basename_localization;
}

void Node::SetLoadStateFilename(std::string &filename, bool frozen_state){
    load_state_filename = filename;
    if(frozen_state) load_frozen_state = true;
    else load_frozen_state = false;
}
//added end;

//added by mafuqiang: start to create map with default topics;
bool Node::HandleStartCreateMap(
        std_srvs::Empty::Request& request,
        std_srvs::Empty::Response& response){

    if(start_create_map_controller){
        if(is_active_trajectory_.size()>0) ResetNode();

        carto::common::MutexLocker lock(&mutex_);
//    TrajectoryOptions options;
//    options = LoadTrajectoryOptions(configuration_directory, configuration_basename);
        CHECK(ValidateTrajectoryOptions(trajectory_options_mafuqiang));
        create_map_trajectory_id = AddTrajectory(trajectory_options_mafuqiang, DefaultSensorTopics());
        start_create_map_controller = false;
    }
    else{
        ROS_ERROR_STREAM("Please first run service 'finish_create_map'.");
    }

  return true;
}

bool Node::HandleFinishCreateMap(
        std_srvs::Empty::Request& request,
        std_srvs::Empty::Response& response){
    carto::common::MutexLocker lock(&mutex_);
    FinishTrajectoryUnderLock(create_map_trajectory_id);
    start_create_map_controller = true;
    return true;
}

bool Node::HandleStartLocalization(
        std_srvs::Empty::Request& request,
        std_srvs::Empty::Response& response){
    carto::common::MutexLocker lock(&mutex_);
    //这个函数接口需要重新设定topic的参数，因为默认建图服务使用默认的topic，通过launch启动节点时
    //已经经过了remap，可以使用，这个函数接口可能需要需要自己设定传感器发布的topic名称；
    //××××应该也不需要重新设定，该node不杀死的话，原来的remap的topic应该还在，可以直接使用×××××××××；

    //因为定位模式参数有调整，因此需要重新加载定位需要的参数文档；
    TrajectoryOptions trajectory_options_localization;// = trajectory_options_mafuqiang;
//    trajectory_options_localization.trajectory_builder_options.pure_localization() = true;
    trajectory_options_localization = LoadTrajectoryOptions(configuration_directory,
                                                            configuration_basename_localization);
    CHECK(ValidateTrajectoryOptions(trajectory_options_localization));
    localization_trajectory_id = AddTrajectory(trajectory_options_localization, DefaultSensorTopics());
    if(localization_trajectory_id > 0){
        for(int i = 0; i < localization_trajectory_id - 1; ++i){
            is_active_trajectory_[i] = false;
        }
    }
...
bool Node::HandleStartLocalizationFromPbstream(
        std_srvs::Empty::Request& request,
        std_srvs::Empty::Response& response){
...
bool Node::HandleFinishLocalization(
        std_srvs::Empty::Request& request,
        std_srvs::Empty::Response& response){
    carto::common::MutexLocker lock(&mutex_);

    FinishTrajectoryUnderLock(localization_trajectory_id);
