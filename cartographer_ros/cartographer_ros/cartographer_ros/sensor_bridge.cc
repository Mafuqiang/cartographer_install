...
//  int length = frame_id.length();
//  for(int i = 0; i < length; i ++)
//  {
//    if(i == 0) continue;
//    new_frame_id.push_back(frame_id[i]);
//  }
//  std::string new_frame_id = frame_id.subString(1, length);
...
//  const std::string newfi(new_frame_id);
//  return newfi;
  return frame_id;
}
...
SensorBridge::SensorBridge(
 ...
 // 将ROS类型的Odom消息格式转换为相对于Tracking frame的carto类型消息格式；
