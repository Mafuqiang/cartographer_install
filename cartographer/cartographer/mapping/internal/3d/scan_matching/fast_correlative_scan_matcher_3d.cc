...
FastCorrelativeScanMatcher3D::Match(
  ...
  const SearchParameters search_parameters{
    ...
    relocation_current_score_controller = false;
...
FastCorrelativeScanMatcher3D::MatchFullSubmap(
  ...
  const SearchParameters search_parameters{
    ...
    relocation_current_score_controller = true;
...
//added by mafuqiang;FastCorrelativeScanMatcher3D::
float GetCurrentMatchFullSubmapScore(){
//    std::cout<<"******************"<<std::endl<<"get current score:"<<current_score<<std::endl<<
//    "************************************************"<<std::endl;
...
FastCorrelativeScanMatcher3D::MatchWithSearchParameters(
  ...
  const std::vector<Candidate3D> lowest_resolution_candidates =
    ...
    if(relocation_current_score_controller)
        current_score = 0.;
  ...
  //added by mafuqiang;
//  const float tmp_score = best_candidate.score;
//  float tmpcur = const_cast<float&>(tmp_score);
//  current_score = tmpcur;
//if(relocation_current_score_controller)
//{
//    std::cout<<"********************"<<std::endl<<"current_score:"<<current_score<<std::endl
//    <<"***********************"<<std::endl;
//    std::cout<<"$$$$$$$$$$$$$$$$$$$"<<best_candidate.score<<"$$$$$$$$$$$$$$$$$$$$"<<std::endl;
//    std::cout<<"@@@@@@@@@@@@@@@@@@@"<<precomputation_grid_stack_->max_depth()<<"@@@@@@@@@@@@@@@@@@@@@@@@@@@"<<std::endl;
//}
...
//          if(current_score > min_score) current_score = 0.;
          if(relocation_current_score_controller && current_score == 0.)
          {
              current_score = const_cast<float&>(candidate.score);
//              std::cout<<"##########################################"<<std::endl;
...
//              std::cout<<"##########################################"<<std::endl;
      ...
      if (low_resolution_score >= options_.min_low_resolution_score()) {
        ...
        if(relocation_current_score_controller)
        {
//            std::cout<<"^^^^^^^^^^^^^^^^^^^^^^low resolution score^^^^^^^^^^^^^^^^^^^^^^^^"<<std::endl;
            current_score = const_cast<float&>(best_candidate.score);
...
//            std::cout<<current_score<<std::endl;
    ...
    best_high_resolution_candidate = std::max(
      ...
      if(best_high_resolution_candidate.score > min_score && relocation_current_score_controller){
          current_score = const_cast<float&>(best_high_resolution_candidate.score);
      }
  }
