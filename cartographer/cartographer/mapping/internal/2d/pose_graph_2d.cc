...
//added by mafuqiang;
        PoseGraphInterface::RelocationResult PoseGraph2D::GetRelocationResult()
        {
            PoseGraphInterface::RelocationResult relocation_result;
            common::MutexLocker locker(&mutex_);
            relocation_result = constraint_builder_.GetRelocationResult();
...
// 初始化全局submap的pose，返回的是这些待插入submap在global中的索引；
std::vector<SubmapId> PoseGraph2D::InitializeGlobalSubmapPoses(
  ...
  if (insertion_submaps.size() == 1) {  // 如果插入的submaps数据为1
    // If we don't already have an entry for the first submap, add one.
    if (submap_data.SizeOfTrajectoryOrZero(trajectory_id) == 0) {   // 如果当前轨迹ID的submap数量为0，说明还没有submap构建；
      if (initial_trajectory_poses_.count(trajectory_id) > 0) {     // 如果有初始位姿输入，则构建初始相对位姿所在的两个轨迹之间的联系；
        trajectory_connectivity_state_.Connect(
            trajectory_id,
            initial_trajectory_poses_.at(trajectory_id).to_trajectory_id, time);    // 连接两个轨迹？？？
      }
      // 将初始姿态加入优化；
      optimization_problem_->AddSubmap(
          ...
          transform::Project2D(ComputeLocalToGlobalTransform(
                               ...
                               insertion_submaps[0]->local_pose()));        // 获得local到global之间的变换，然后插入的submap在local下的位姿，得到在global下的位姿；
    }
    // 到这里最新的轨迹下的submap应该至少为1；
    CHECK_EQ(1, submap_data.SizeOfTrajectoryOrZero(trajectory_id));
    const SubmapId submap_id{trajectory_id, 0}; // 该轨迹下的 0 submap；
    CHECK(submap_data_.at(submap_id).submap == insertion_submaps.front());  // 两个其实都是1；
    return {submap_id};
  }
  CHECK_EQ(2, insertion_submaps.size());    // 插入的submap = 2？
  const auto end_it = submap_data.EndOfTrajectory(trajectory_id);   // 最后的应该是时间上最新的？
  CHECK(submap_data.BeginOfTrajectory(trajectory_id) != end_it);
  const SubmapId last_submap_id = std::prev(end_it)->id;    // 最新的前一帧；
  if (submap_data_.at(last_submap_id).submap == insertion_submaps.front()) {    // 优化器内的最新的和待插入的第一个ID相等？？？
...
void PoseGraph2D::ComputeConstraint(const NodeId& node_id,
  ...
  if (node_id.trajectory_id == submap_id.trajectory_id ||
    ...
    // local search window.
    const transform::Rigid2d initial_relative_pose =    // 同一个轨迹下，或短时间内建立过约束的不同轨迹间，可以给定初始约束姿态；
        optimization_problem_->submap_data().at(submap_id).global_pose.inverse() *
        optimization_problem_->node_data().at(node_id).global_pose_2d;

//    if(node_id.trajectory_id == submap_id.trajectory_id)
//        std::cout<<"first: xxxxxxxxxxxxxxxx**node_id_trajectory == submap_id_trajectory**xxxxxxxxxxxxxxxx"<<std::endl;
//    else
//        std::cout<<"first: xxxxxxxxxxxxxxxx**node_id_trajectory != submap_id_trajectory**xxxxxxxxxxxxxxxx"<<std::endl;

    // 添加约束，通过分支界定的scan match方法在初始位姿附近按照搜索窗口参数进行匹配；
...
//      if(node_id.trajectory_id == submap_id.trajectory_id)
//          std::cout<<"second: xxxxxxxxxxxxxxxx**node_id_trajectory == submap_id_trajectory**xxxxxxxxxxxxxxxx"<<std::endl;
//      else
//          std::cout<<"second: xxxxxxxxxxxxxxxx**node_id_trajectory != submap_id_trajectory**xxxxxxxxxxxxxxxx"<<std::endl;

      // 添加约束，通过分支界定的scan match方法在全局map的中心搜索全图，找到匹配；
...
void PoseGraph2D::ComputeConstraintsForNode(
  ...
  // 初始化全局submap的pose：轨迹ID，时间戳，插入的submaps，返回的是待插入的submap在global中的索引ID；
  ...
  for (size_t i = 0; i < insertion_submaps.size(); ++i) {
    ...
    CHECK(submap_data_.at(submap_id).state == SubmapState::kActive);    // 后端的优化窗口内属于kActive？这个submap_id的submap状态必然是
                                                                        // kActive，因为其实是还没有插入后端优化的submap，即使后端优化的node为0，也要等到插入以后；
    submap_data_.at(submap_id).node_ids.emplace(node_id);

    const transform::Rigid2d constraint_transform = // 优化窗口内的变换？？这是应该是约束变换
    ...
    constraints_.push_back(Constraint{submap_id,
                                      ...
                                      Constraint::INTRA_SUBMAP});   // intra：node被插入了submap；
  }

  // 遍历所有的submap_data_，选择其中状态为kFinished的submap计算回环优化和重定位优化边；
  for (const auto& submap_id_data : submap_data_) {
    if (submap_id_data.data.state == SubmapState::kFinished) {
        // 一个在后端优化线程中finished的submap，应该不包含最新node id，然后给他们计算约束关系；
...
transform::Rigid3d PoseGraph2D::ComputeLocalToGlobalTransform(
  ...
  // 如果全局submap的pose索引，开始与结束相等，说明该轨迹刚开始运行，还没有submap生成，
  // 此时还要局部到全局的变换，因该是加载了轨迹，因此，判定是否使用初始位姿，然后返回；
  if (begin_it == end_it) {
  ...
  // 否则，给出最新的优化过后的submap id（即最后一个），然后利用这个优化过的submap的全局位姿
  const SubmapId last_optimized_submap_id = std::prev(end_it)->id;
  // Accessing 'local_pose' in Submap is okay, since the member is const.
  return transform::Embed3D(
             global_submap_poses.at(last_optimized_submap_id).global_pose) *
         submap_data_.at(last_optimized_submap_id)          // 这个submap_data_什么时候初始化的？
             .submap->local_pose()                          // local的逆乘以global pose；
             .inverse();
}
...
void PoseGraph2D::TrimmingHandle::MarkSubmapAsTrimmed(
  ...
  for (const Constraint& constraint : parent_->constraints_) {
    if (constraint.tag == Constraint::Tag::INTRA_SUBMAP &&
      ...
      nodes_to_retain.insert(constraint.node_id);   // 如果约束的submap索引不等于给定的索引，则保留？？
    }
  }
  ...
  std::set<NodeId> nodes_to_remove;
  {
    ...
    for (const Constraint& constraint : parent_->constraints_) {
      if (constraint.submap_id == submap_id) {
          // 这块儿什么意思？
        if (constraint.tag == Constraint::Tag::INTRA_SUBMAP &&
            nodes_to_retain.count(constraint.node_id) == 0) {   //vector.count返回该元素的个数；
  ...
  CHECK(parent_->submap_data_.at(submap_id).state == SubmapState::kFinished);   // 这个submap必须已经完成了；
