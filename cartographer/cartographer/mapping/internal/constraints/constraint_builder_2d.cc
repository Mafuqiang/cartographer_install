...
//
//float getPurgeLocalizationMatchScore(){
//    return purge_localization_score;
//}
...
// 使用两帧之间的scan match作为匹配结果；
void ConstraintBuilder2D::MaybeAddConstraint(
    const SubmapId& submap_id, const Submap2D* const submap,
    const NodeId& node_id, const TrajectoryNode::Data* const constant_data,
    const transform::Rigid2d& initial_relative_pose) {
    // 如果约束节点之间的平移距离大于最大限制距离，则取消建立约束；
...
//added by mafuqiang;
        PoseGraphInterface::RelocationResult ConstraintBuilder2D::GetRelocationResult(){
            PoseGraphInterface::RelocationResult relocation_result;
            common::MutexLocker locker(&mutex_);
            relocation_result.max_score = max_score;
            relocation_result.min_score = min_score;
            relocation_result.relocation_num = relocation_num;
            relocation_result.successed_num = successed_num;

//            std::cout<<"**************************************"<<std::endl;
//            std::cout<<"          mafuqiang                   "<<std::endl;
//            std::cout<<"max_score:"<<max_score<<std::endl;
//            std::cout<<"min_score:"<<min_score<<std::endl;
//            std::cout<<"rel_numbe:"<<relocation_num<<std::endl;
//            std::cout<<"suc_numbe:"<<successed_num<<std::endl;
//            std::cout<<std::endl;
//            std::cout<<"relocation_result.max_score:"<<relocation_result.max_score<<std::endl;
//            std::cout<<"relocation_result.min_score:"<<relocation_result.min_score<<std::endl;
//            std::cout<<"relocation_result.relocation_num:"<<relocation_result.relocation_num<<std::endl;
//            std::cout<<"relocation_result.successed_num:"<<relocation_result.successed_num<<std::endl;
...
//added by mafuqiang;
//        int ConstraintBuilder2D::GetRelocationNum(){
//    common::MutexLocker locker(&mutex_);
//    return relocation_num;
//}
//
//        int ConstraintBuilder2D::GetSuccessedNum(){
//            common::MutexLocker locker(&mutex_);
//    return successed_num;
//}
//
//        float ConstraintBuilder2D::GetRelocationMaxScore(){
//            common::MutexLocker locker(&mutex_);
//    return max_score;
//}
//
//        float ConstraintBuilder2D::GetRelocationMinScore(){
//            common::MutexLocker locker(&mutex_);
//    return min_score;
//}
...
void ConstraintBuilder2D::ComputeConstraint(
    ...
    // initial_relative_pose是node到submap的相对位姿；
    // initial_pose是在世界坐标系下的姿态；
  ...
  // 3. Refine.
  if (match_full_submap && scan_matching_controller) {
    kGlobalConstraintsSearchedMetric->Increment();
    if (submap_scan_matcher.fast_correlative_scan_matcher->MatchFullSubmap(
            constant_data->filtered_gravity_aligned_point_cloud,
            options_.global_localization_min_score(), &score, &pose_estimate)) {    // 如果分枝定界的scan matching有结果了
      ...
      //std::cout<<"full submap: *****************pose_estimate(x,y):"<<pose_estimate.translation().x()<<" "<<pose_estimate.translation().y()<<
      //"*********************"<<std::endl;


//        purge_localization_score = score;
        relocation_num ++;
        successed_num ++;
        if(score > max_score)
        {
        ...
        //std::cout<<"full submap failed: *****************pose_estimate(x,y):"<<pose_estimate.translation().x()<<" "<<pose_estimate.translation().y()<<
                 //"*********************"<<std::endl;
//        purge_localization_score = -1.;

      return;   // 如果匹配的不好，就直接返回空；
    }
  } else if(!match_full_submap){
    kConstraintsSearchedMetric->Increment();
    if (submap_scan_matcher.fast_correlative_scan_matcher->Match(
            initial_pose, constant_data->filtered_gravity_aligned_point_cloud,
            options_.min_score(), &score, &pose_estimate)) {    // 这个是在初始姿态处进行匹配，上面是在全局进行匹配；
      // We've reported a successful local match.
      CHECK_GT(score, options_.min_score());
      kConstraintsFoundMetric->Increment();
      kConstraintScoresMetric->Observe(score);

      if(scan_matching_controller)
          scan_matching_controller = false;
      //std::cout<<"Match: ***********pose_estimate(x,y):"<<pose_estimate.translation().x()<<" "<<pose_estimate.translation().y()<<
      //"*******************"<<std::endl;
