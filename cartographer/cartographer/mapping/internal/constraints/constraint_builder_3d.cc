...
//added by mafuqiang;
    PoseGraphInterface::RelocationResult ConstraintBuilder3D::GetRelocationResult(){
        common::MutexLocker locker(&mutex_);
    ...
    relocation_result.current_score = current_score;
...
//    std::cout<<"cur_score:"<<current_score<<std::endl;
...
//    std::cout<<"relocation_result.current_score:"<<relocation_result.current_score<<std::endl;
...
//    int ConstraintBuilder3D::GetRelocationNum(){
//        common::MutexLocker locker(&mutex_);
//        return relocation_num;
//    }
//
//    int ConstraintBuilder3D::GetSuccessedNum(){
//        common::MutexLocker locker(&mutex_);
//        return successed_num;
//    }
//
//    float ConstraintBuilder3D::GetRelocationMaxScore(){
//        common::MutexLocker locker(&mutex_);
//        return max_score;
//    }
//
//    float ConstraintBuilder3D::GetRelocationMinScore(){
//        common::MutexLocker locker(&mutex_);
//        return min_score;
//    }
...
//added by mafuqiang;
//    current_score = submap_scan_matcher.fast_correlative_scan_matcher->GetCurrentMatchFullSubmapScore();
      current_score = scan_matching::GetCurrentMatchFullSubmapScore();
    std::cout<<"***********current_score************"<<std::endl<<current_score<<std::endl;
...
//added by mafuqiang;
        ...
        std::cout<<"*****mafuqiang*****relocation num:"<<relocation_num<<std::endl;
        std::cout<<"*****mafuqiang*****successed num:"<<successed_num<<std::endl;
        std::cout<<"mafuqiang*****:score:"<<match_result->score<<std::endl;

        if(match_result->score > max_score)
        {
            max_score = match_result->score;
        }
        else //if(match_result->score < min_score &&
                //match_result->score >= options_.global_localization_min_score())
        {
            min_score = match_result->score;
        }
    } else {
        std::cout<<"*****mafuqiang*****relocation num:"<<relocation_num<<std::endl;
        std::cout<<"*****mafuqiang*****successed num:"<<successed_num<<std::endl;
//        current_score = scan_matching::GetCurrentMatchFullSubmapScore();
        std::cout<<"*****mafuqiang: relocation failed.******"<<std::endl;
      return;
    }
  } else {
      ++loop_match_num;
    ...
    if (match_result != nullptr) {
      ...
      kConstraintLowResolutionScoresMetric->Observe(
          match_result->low_resolution_score);
//      std::cout<<"loop_match_num: success."<<std::endl;
    } else {
//        std::cout<<"loop_match_num: failed."<<std::endl;
      return;
    }
  }
  {
